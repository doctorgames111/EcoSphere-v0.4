document.addEventListener('DOMContentLoaded', function() {
  ...
  function validateField(input) {
    ...
    if (name === 'username') {
      if (value.length < 4) {
        displayFieldError(input, 'Username must be at least 4 characters');
        return false;
      }
      if (value.length > 20) {
        displayFieldError(input, 'Username cannot exceed 20 characters');
        return false;
      }
      if (/\s/.test(value)) {
        displayFieldError(input, 'Username cannot contain spaces');
        return false;
      }
      // uniqueness checked asynchronously on blur / server-side during submit
    }
    if (name === 'firstName' || name === 'lastName') {
      if (value.length > 20) {
        displayFieldError(input, 'Name cannot exceed 20 characters');
        return false;
      }
    }
    if (name === 'email') {
